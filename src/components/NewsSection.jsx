...
function ImageCard({ title, image, readMore = true, titlePosition = "bottom", className = "" }) {
  const [hovered, setHovered] = useState(false);

  return (
    <div className={"relative overflow-hidden rounded-2xl " + className} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <img
        src={image}
        alt={title}
      ...
      {titlePosition === "bottom" ? (
        <>
          ...
          <div className="absolute bottom-0 left-0 right-0 flex items-end justify-between gap-2 p-4">
            {readMore && (
              <a href="#" className="shrink-0 text-xs font-medium text-white/80 hover:text-white">
                Read More
              </a>
            )}
          </div>
        ...
        <>
          ...
          {readMore && (
            <div className="absolute bottom-4 right-4">
              <a href="#" className="text-xs font-medium text-white/80 hover:text-white">
                Read More
              </a>
            </div>
          )}
    ...
    <section className="w-full py-12">
      <div className="mx-auto max-w-[90%] px-4 md:px-6">
        <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{ once: false, margin: "0px 0px -20% 0px" }}>
          <motion.h2 className="!mb-[66px] !text-[36px] !font-bold !font-black !tracking-tight !text-black text-left" variants={slideLeft}>
            News & Events
          </motion.h2>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-[5fr_2fr_2fr] md:items-stretch">
            ...
            <div className="flex flex-col gap-4">
              <motion.div variants={slideLeft}>
                <ImageCard title="Her Majesty Gyalyum Dorji Wangmo Wangchuck visited GCIT" image={n1} className="h-64" titlePosition="top" />
              </motion.div>
              <motion.div className="grid grid-cols-2 gap-4" variants={slideLeft}>
                <ImageCard title="BUSF Regional Tournament 2025 – at NRC!" image={n4} className="h-100" titlePosition="top" />
                <ImageCard title="Training on University Teaching-Learning Pedagogy" image={n5} className="h-100" titlePosition="top" />
              </motion.div>
            </div>

            {/* Middle column */}
            <motion.div className="flex flex-col gap-4" variants={slideUp}>
              <ImageCard title="The Royal Family graced GCIT's 2025 Capstone Tech Innovation Showcase." image={n2} titlePosition="top" className="min-h-64 flex-1" />
              <div className="rounded-2xl bg-[#0d1b3e] p-5 text-white">
                <h3 className="text-[14px] font-bold leading-snug">JOIN GCIT'S FAMILY! | Looking for an Assistant Lecturer in Interactive Design & Development</h3>
                <p className="mt-3 text-xs text-white/50">Registration closes: December 05, 2025</p>
                <div className="mt-4 flex justify-end">
                  <a
                    href="#"
                    className="rounded-full border border-[#f48b1a] px-5 py-1.5 text-xs font-semibold text-[#f48b1a] transition-colors hover:bg-[#f48b1a] hover:text-white"
                  >
                    Apply Now
                  </a>
                </div>
            ...
            <motion.div className="flex flex-col gap-4" variants={slideRight}>
              <div className="rounded-2xl bg-gray-100 p-5">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="text-base font-bold text-black">AI & Data Science Hackathon</h3>
                    <p className="mt-2 text-sm text-gray-500">Registration closes: November 20, 2025</p>
                  </div>
                </div>
              </div>
              <ImageCard title="Welcome Show" image={n3} titlePosition="top" className="h-60" />
